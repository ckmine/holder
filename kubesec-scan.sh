...
scan_result=$(curl -sSX POST --data-binary @"blue.yml" https://v2.kubesec.io/scan)
scan_message=$(curl -sSX POST --data-binary @"blue.yml" https://v2.kubesec.io/scan | jq .[0].message -r ) 
scan_score=$(curl -sSX POST --data-binary @"blue.yml" https://v2.kubesec.io/scan | jq .[0].score ) 
